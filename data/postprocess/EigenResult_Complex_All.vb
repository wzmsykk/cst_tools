...
End Function
Function EigenResult_Complex(iModeNumber As Integer,queryKey As String,axis As Integer,ByRef iUVWvalue() As Double) As Double
    ...
    Dim needVotage As Boolean
    If queryKey="R over Q" Or queryKey="Shunt Inpedence" Then
        needVotage=True
    End If

    Dim vwfull As Double
    If needVotage Then
    ...
    End If 'VOTAGE LOOP ENDS'
    Dim cst_value As Double
    if queryKey="Q-Factor" Then
        cst_value=q_cst
    ElseIf queryKey="Total Loss" Then
        cst_value=pw_cst
    ElseIf queryKey="Total Energy" Then
        cst_value=eng_cst
    ElseIf queryKey="R over Q" Then
        cst_value = vwfull*vwfull/(2*Pi* frq_cst*Units.GetFrequencyUnitToSI() * eng_cst) 'R_over_Q value
    ElseIF queryKey="Shunt Inpedence" Then
        cst_value = vwfull*vwfull/pw_cst'Shunt Inpedence value
    End IF
    
    
    
    ReportInformation(cst_value)
    EigenResult_Complex=cst_value
End Function

Function EigenResult_Complex_output(iModeNumber As Integer,queryKey As String,axis As Integer,xoffset As Double,yoffset As Double,zoffset As Double, outputDir As String, outputName As String)
    Dim roq As Double
    ...
    uvw(2,1)=yoffset
    uvw(3,1)=zoffset
    uvw(axis,1)=0 'RECOVER
    
    roq=EigenResult_Complex(iModeNumber,queryKey,axis,uvw)
    Open(outputDir & outputName) For Output As #1
    Print #1,"Result Name"
    Print #1,"EigenResult_Complex" 'Name
    Print #1,"Result Type"
    Print #1,queryKey
    Print #1,"ModeNumber"
    Print #1,iModeNumber
...
Function EigenResult_Complex_All_output(queryKey As String,axis As Integer,xoffset As Double,yoffset As Double,zoffset As Double, outputDir As String, outputName As String)
    ...
    For iModeNumber = 1 To EigenmodeSolver.GetNumberOfModesCalculated
        result=EigenResult_Complex(iModeNumber,queryKey,axis,uvw)
