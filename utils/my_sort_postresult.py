    ...
    for file in files:
        ...
        if "frequency000000_" in m:
            ...
            samples = samples.append(
                pd.DataFrame([list(mode1)], columns=input_name + output_name)
            )
            samples = samples.append(
                pd.DataFrame([list(mode2)], columns=input_name + output_name)
            )
            samples = samples.append(
                pd.DataFrame([list(mode3)], columns=input_name + output_name)
            )
            # time.sleep(1)
else:
