...
class myAlg01(object):
    def __init__(self, manager, params):
        ...
        self.input_name = ['Leq', 'R3_left', 'R3_right', 'Req', 'cocave', 'concave_L', 'nose', 'r0_left', 'r0_right', 'r1_left', 'r1_right', 'r2_left', 'r2_right']
        
        

        # self.input_name_calculate = ['l', 'ltube', 'number']
        self.output_name = ['global result', 'frequent', 'R divide Q', 'Q-factor', 'shunt impedance', 'voltage', 'total loss']
        self.dimension_input = len(self.input_name)
        self.dimension_output = len(self.output_name)
        
        #encoding
        self.bound = [[250,280],[0,125],[0,125],[214,254],[32,33],[32,33],[0,40],[0,15],[0,15],[0,25],[0,25],[0,100],[0,100],]
    ...
    def get_nonsense_len(self, ends):
        i = 0
#       print(ends[:10])
        while (ends[i]==' ' or ends[i]==',' or ends[i]=='\t' or ends[i]==']' or ends[i]=='[' or ends[i]=='('):
            i += 1
        return i
    ...
    def get_pramater(self, location):
        ...
        while present_position_start<len_text-1:
            delta_len = self.find_line_end(ends[present_position_start:])#找到这一行的终止位置（判断换行符）
            ...
            if "X_mean" in string:
                ...
                x_mean = []
                for i in ends[present_position_start:present_position_end-1].split():
                    # print(i)
                    try:
    ...
    def get_y_trans_r_aprallel(self, xs, run_count):
        ...
        if len(r)==1:
           ...
           return np.append(np.array(r).reshape(self.generation_num,1),np.array(y).reshape(self.generation_num,self.dimension_output-1),axis=1) 
...
#         print(samples)
    ...
    def get_sorted(self, samples_fresh):
        samples_fresh["nearest_to_500M"] = abs(np.array(samples_fresh["frequent"])-499.65)
        samples_sorted = samples_fresh.sort_values(by = "nearest_to_500M", ascending = True).head(self.generation_num*2)
        # self.write_many("samples_sorted_nearest_to_500M", samples_sorted)
        samples_sorted = samples_sorted.sort_values(by = "shunt impedance", ascending=False).head(self.generation_num)
        # self.write_many("samples_sorted_nearest_to_500M", samples_sorted)
    ...
    def get_p_variated(self, samples_fitted):
        ...
        for i in range(self.generation_num):
            p_variated.append(1-np.float(samples_fitted["fitness"].iloc[i])/np.sum(np.array(samples_fitted["fitness"])))
        samples_fitted["p_variated"] = np.array(p_variated).reshape(self.generation_num,1)
...
#             print(self.bound[i][1], self.bound[i][0])
            samples_new_parameter[i] = random.random()*(bound[i,1] - bound[i,0]) + bound[i,0]
...
#         print("bound is: \n",bound)
        samples_encoded = (samples_input_value - bound[:,0])/(bound[:,1] - bound[:,0])
...
#         print(i, random_num, samples_fitted)
    ...
    def get_next_generation(self, samples_choosed, samples_inheritanced, samples_variated):
        samples_all = pd.concat([samples_choosed, samples_inheritanced, samples_variated])
...
#         print("YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY:\n",samples_sorted)
        samples_fitted = self.get_fitness(samples_sorted).reset_index(drop = True).drop(["p_choosed", "fitness"], 1)
#         print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n",samples_fitted)
    ...
    def judge_end(self, samples_fresh):
        ...
        if self.count>self.end_num:#samples_fresh[0].all < 0.001:
            ...
            self.write_end(np.array(samples_sorted), self.output_name, "the_best_sample")
            return False
        else:
#             print(self.count)
...
#                 print(np.array(samples_fresh[self.input_name]))
                samples_new_parameter = self.get_samples_new_parameter(np.array(samples_fresh[self.input_name]))
        ...
        samples_fitted = self.get_fitness(samples_sorted).drop(["p_choosed", "fitness", "p_variated"],1)
#         print(samples_fitted)
...
#         print(samples_fitted)
        ...
        for i in range(self.generation_num-5):
            num = self.get_choosed_sample(random.random(), np.array(samples_fitted["p_choosed"]).reshape(self.generation_num,))
#             print(num)
        ...
        try:
            samples_choosed = samples_choosed.drop(["p_choosed", "fitness", "p_variated"], 1)
        except:
...
#         print(samples_sorted)
        output_value = np.array(samples_sorted[self.output_name]).reshape(self.generation_num,self.dimension_output)[:,0]
#         print(output_value)
...
#         print(fitness.shape)
        ...
        #add p
        p_choosed = []
        p_choosed.append(np.float(samples_sorted["fitness"].iloc[0])/np.sum(np.array(samples_sorted["fitness"])))
#         print(np.sum(np.array(samples_fitted["fitness"].iloc[0])), np.sum(np.array(samples_fitted["fitness"].iloc[1])), np.sum(np.array(samples_fitted["fitness"].iloc[:2])))
        for i in range(1, self.generation_num):
            p_choosed.append(np.sum(np.array(samples_sorted["fitness"].iloc[:i+1]))/np.sum(np.array(samples_sorted["fitness"])))
        samples_sorted["p_choosed"] = np.array(p_choosed).reshape(self.generation_num,1)
#         print(samples_sorted)
...
#         print(samples_choosed[mother,:])
        if u3<0.5:
            sample_new = u1*samples_choosed.loc[father]+(1-u1)*samples_choosed.loc[mother]
        else:
            sample_new = u2*samples_choosed.loc[father]+(1-u2)*samples_choosed.loc[mother]
        return sample_new
...
#         print(os.getcwd()) #获取当前工作路径
        ...
        name = self.location + self.relative_location + "picture\\" + title + "_" + str(self.count) + ".csv"
#         print("name:",name)
        data.to_csv(name,index=True,sep=',')
    
    def start(self):
        start_time = time.time()
        self.state_y0 = np.array(self.w.runWithParam(self.input_name,self.state_x0, "normal_result"))
...
#             print("samples_fitted:\n",samples_fitted)
            ...
            samples_fresh = self.get_next_generation(samples_choosed, samples_inheritanced, samples_variated)
            # print("samples_fresh::\n",samples_fresh)
