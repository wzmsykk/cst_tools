...
class gredientDecend(object):
    ...
    def deltaY_Norm_5point(self, pam0, pam1, pam2, pam3, pam4, nor):
        ...
        b = (
            -0.3
            * (48 * pam1[1] - 25 * pam0[1] - 36 * pam2[1] + 16 * pam3[1] - 3 * pam4[1])
            / nor[1]
            - 0.15
            * (48 * pam1[2] - 25 * pam0[2] - 36 * pam2[2] + 16 * pam3[2] - 3 * pam4[2])
            / nor[2]
            - 0.05
            * (48 * pam1[4] - 25 * pam0[4] - 36 * pam2[4] + 16 * pam3[4] - 3 * pam4[4])
            / nor[4]
        )
    ...
    def pGradFromR0_norm_5point(self, x0, r0, step, lse, indc, nor, resultDir):
        ...
        for j in range(len(indc)):
            ...
            pa = self.w.createCSTBatchByName(
                tl, tl[indc[j]]["name"] + str(1), resultDir
            )
            pa2 = self.w.createCSTBatchByName(
                tl2, tl2[indc[j]]["name"] + str(2), resultDir
            )
            pa3 = self.w.createCSTBatchByName(
                tl3, tl3[indc[j]]["name"] + str(3), resultDir
            )
            pa4 = self.w.createCSTBatchByName(
                tl4, tl4[indc[j]]["name"] + str(4), resultDir
            )
            self.w.runcst(pa)
            self.w.runcst(pa2)
            self.w.runcst(pa3)
    ...
    def do(self, resultDir, iconf, nordata):
        ...
        while i <= self.maxcount:
            ...
            pgrad = self.pGradFromR0_norm_3point(
                x0, r0, self.step, lse, pt, nor, resultDir
            )
    ...
    def start(self):
        ...
        stps = self.genRandomStartPs(
            self.startpoints, len(pt), ubounds=ubounds, lbounds=lbounds
        )
        x0 = np.zeros(len(pt))
        for j in range(len(pt)):
            x0[j] = lse[pt[j]]["value"]
        ...
        for i in range(len(stps)):
            ...
            print(
                "beginning with startpoint", i, "is", stps[i], file=self.log, flush=True
            )
        ...
        print(
            "Best result y is", ymax, "with startpoint", ix, file=self.log, flush=True
        )
