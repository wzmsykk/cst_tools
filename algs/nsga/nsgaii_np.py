...
from random import random, seed, shuffle, choices
...
class nsgaii_var:
    ...
    def __init__(self, value) -> None:
        ...
        self.pset: Set[
            nsgaii_var
        ] = set()  ### set of var domed by p ### By Ref so its memory safe
        self.done = False
        self.sorted = False
        self.n = 0
        self.crowed_dis = 0
        self.crowed_dis_calc_done = False
        self.id = nsgaii_var.id
        nsgaii_var.id += 1
...
def fnds(vallist: List[nsgaii_var]):  ## Fast non dominated sort
...
def crossover_real_SBX_np(par1, par2):

    uarray = np.random.random(nval)
    ...
    val1=np.clip(val1,min_realvar,max_realvar)
    val2=np.clip(val2,min_realvar,max_realvar)
    child1 = nsgaii_var(val1)
    child2 = nsgaii_var(val2)
    return child1, child2


def mutation_real_np(ind: nsgaii_var):

    rnd = np.random.random(nval)
    barray = np.less(rnd, pmut_real)
    rnd2 = np.random.random(nval)
    barray2 = np.less_equal(rnd2, 0.5)
    nbarray2 = np.logical_not(barray2)
    v = ind.value
    vl = np.array(min_realvar)
    vu = np.array(max_realvar)
    delta1 = (v - vl) / (vu - vl)
    delta2 = (vu - v) / (vu - vl)
    ...
    val1 = 2.0 * rnd2 + (1.0 - 2.0 * rnd2) * xy1 **(eta_m + 1.0)
    deltaq1 = val1** mut_pow - 1.0

    xy2 = (1.0 - delta2)*nbarray2
    val2 = 2.0 * (1.0 - rnd2) + 2.0 * (rnd2 - 0.5) * xy2** (eta_m + 1.0)
    deltaq2 = 1.0 - val2** mut_pow

    v = v + (deltaq1 * barray2 + deltaq2 * nbarray2) * (vu - vl)
    np.clip(v, vl, vu, v)
    v = v * barray
    v=np.clip(v,min_realvar,max_realvar) ###boundary check
    ind.value = v


def tournament(ind1: nsgaii_var, ind2: nsgaii_var):
...
def offspring_gen(inpop: List[nsgaii_var]):
    offspop = []
    quart_size = floor(len(inpop) / 4)
    shuffle(inpop)
    for i in range(quart_size):
        par1 = tournament(inpop[i], inpop[i + 1])
        par2 = tournament(inpop[i + 2], inpop[i + 3])
        child1, child2 = crossover_real_SBX_np(par1, par2)
    ...
    if extra > 0:
        ...
        for i in range(extra):
            par1 = tournament(ichoice[i], ichoice[i + 1])
            par2 = tournament(ichoice[i + 2], ichoice[i + 3])
            child1, child2 = crossover_real_SBX_np(par1, par2)
            rnd = random()
            if rnd <= 0.5:
                offspop.append(child1)
    ...
    for child in offspop:
        mutation_real_np(child)
    #print("offspring pop",len(offspop))
    return offspop


def random_sampling_LHS_np(nsamples):
...
def nsgaii_np(model, fnds_callback=None):  ##
    ### fnds_callback for mid output

    valarr = random_sampling_LHS_np(popsize)
    val_width=max_realvar-min_realvar
    valarr=valarr*val_width+min_realvar
    ...
    for igen in range(generation):
        ...
        childpoplist = offspring_gen(poplist)
        
        for ind in childpoplist:
            ind.setResult(list(imodel(ind.value)))
        ###DONE
        poplist = poplist + childpoplist
        fndsresult = fnds(poplist)
...
def nsgaii_test():
    ...
    for model in models:
        ...
        poplist = nsgaii_np(model, fnds_callback=icallback)
        result = fnds(poplist)
        endtime = time.time()
