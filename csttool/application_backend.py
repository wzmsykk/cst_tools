...
class CstApplicationBackend:
    ...
    # Stable application API. The historical camelCase methods above remain
    # private compatibility details until the algorithm layer is migrated.
    def select_project_directory(self, path):
        self.setProjectDir(path)

    def select_cst_file(self, path):
        self.setCSTFilePath(path)

    def get_postprocess_settings(self):
        return self.getCurrPostProcessList()

    def update_postprocess_settings(self, values):
        self.setCurrPostProcessList(values)

    def get_algorithm_settings(self):
        return self.getAlgAttrs()

    def update_algorithm_settings(self, values):
        self.setAlgAttrs(values)

    def initialize_run(self, start_from_existing, safe_mode, worker_count):
        self.setFlags(start_from_existing, safe_mode)
        self.setWorkerCount(worker_count)
        if self.wininit() is False:
            raise RuntimeError("CST 环境初始化失败")

    def prepare_run(self):
        if self.setRunInfos() == 0:
