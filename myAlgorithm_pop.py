...
class myAlg01(myAlg):
    def __init__(self, manager=None, params=None):
        ...
        self.accu_list = pd.DataFrame([[0,1500,1e-5],
                                       [1500,4100,1e-4]],columns=["f_down","f_up","accuracy"])
        self.cell_list =  pd.DataFrame([[0,1300,20],
                                       [1300,2000,15],
...
#         print("name:",name)
    ...
    def get_3_modes(self,title):
        ...
        samples = samples.append(pd.DataFrame([list(mode1)], columns=self.csv_input_name+self.output_name))
        samples = samples.append(pd.DataFrame([list(mode2)], columns=self.csv_input_name+self.output_name))
        print("samples\n",samples)
    ...
    def start(self):
        ...
        if self.continue_flag[0]==0:
            ...
            samples = pd.read_csv(self.relative_location + "all_value_"+str(fmin).replace(".", "-") + ".csv")
        ...
        while fmin<self.end_frequency:
            ...
            while satisfy_flag==False:
                ...
                print((self.accu_list["f_down"]<=fmin) & (self.accu_list["f_up"]>=fmin))
