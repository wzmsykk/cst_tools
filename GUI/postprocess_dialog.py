...
class myPPSDialog(QDialog,Ui_PostProcessSettingDialog):
    ...
    def savePPSJson(self):
        ...
        jsonsavePath,Ok =QFileDialog.getSaveFileName(self,"选取保存位置",filter=filt,initialFilter="JSON(*.json)")
        lst=[]
    ...
    def loadPPSJson(self):
        filt="所有文件(*.*);;JSON(*.json);;TXT(*.txt)"
        jsonsavePath,Ok =QFileDialog.getOpenFileName(self,"选取JSON位置",filter=filt,initialFilter="JSON(*.json)")
        try:
